...
def fileClick():
    global image_path
    image_path = filedialog.askopenfilename(initialdir=data_dir, title="Select a File", filetypes=(
        ("jpg files", "*.jpg*"), ("JGP files", "*.JPG*"), ("png files", "*.png*"), ("PNG files", "*.PNG*"), ("All files", "*.*")))
...
def submit(clicked, gender_val, sam_val, lefthand_val, righthand_val):
    ...
    if isNewDancer:
        print(str(Dancer_name_entry.get())+' ' +
              Dancer_age_entry.get()+' '+gender_val.get())
        if gender_val.get() == 'Male':
            _dancer_gender = 0
        else:
            _dancer_gender = 1
        dancer_in_img = OEM.add_new_dancer_instance(
            Dancer_name_entry.get(), int(Dancer_age_entry.get()), _dancer_gender)
    else:
        print(clicked.get())
        dancer_in_img = OEM.onto.search_one(hasDancerId=int(
            clicked.get().split('\t')[0].split(':')[1]))
    ...
    if isSamyukta:
        ...
        img_static_desc = OEM.add_new_static_instance(
            _dance_name=Dance_name_entry.get(), _is_samyukta=isSamyukta, _samyukta=sam_id)
    else:
        ...
        img_static_desc = OEM.add_new_static_instance(
            _dance_name=Dance_name_entry.get(), _is_samyukta=isSamyukta, _left=l_id, _right=r_id)
...
if __name__ == '__main__':
    ...
    file_explore = Button(path_selector, text="Browse...",
                          command=partial(fileClick))
    file_explore.grid(column=2, row=0, padx="5", pady="5")

    image_frame = Frame(root, highlightthickness=2,
    ...
    dancer_frame = Frame(root, highlightthickness=2,
                         highlightbackground='black')
    ...
    select_button = Button(
        dancer_frame, text='Select a dancer from records', command=show_select_menu, width=70)
    select_dancer_frame = Frame(dancer_frame)

    enter_dancer_button = Button(
        dancer_frame, text='Add new dancer', command=show_add_menu, width=70)
    enter_dancer_frame = Frame(dancer_frame)
    Dancer_name_label = Label(enter_dancer_frame, text="Dancer Name")
    # ,padx=10,pady=10,ipady=10)
    Dancer_name_label.grid(row=0, column=0, padx=5, pady=5)
    ...
    # ,padx=10,pady=10,ipady=5)
    Dancer_age_entry.grid(row=1, column=1, padx=5, pady=5, ipady=5)
    ...
    select_dancer_frame = Frame(dancer_frame)
    dancer_options = ['ID:'+str(dancer.hasDancerId)+'\tNames:'+str(dancer.hasDancerName)+'\tAge:'+str(
        dancer.hasDancerAge)+'\tGenger:'+dancer.hasDancerGender for dancer in OEM.onto.Dancer.instances()]
    clicked = StringVar()
    clicked.set('--Select--')
    ...
    dancer_dropdown.grid(column=1, row=0, padx=5,
                         pady=5)  # , padx="5", pady="5"
    update_name_label = Label(
        select_dancer_frame, text="Add/Update\nDancer Name")
    # ,padx=10,pady=10,ipady=10)
    update_name_label.grid(row=1, column=0, padx=5, pady=5)
    ...
    hastamudra_frame = Frame(root, highlightthickness=2,
                             highlightbackground='black')
    ...
    asam_options = ['ID:'+str(mudra.hasMudraId)+'\t'+str(mudra.name).split('_')[0]
    ...
    sam_options = ['ID:'+str(mudra.hasMudraId)+'\t'+str(mudra.name).split('_')[0]
    ...
    submit_button = Button(root, text='Submit', command=partial(
        submit, clicked, gender_val, sam_val, lefthand_val, righthand_val))
